    ...
    if systems:
        for sys in systems:
            ...
            if delete_pending:
                ...
                st.markdown(
                    f"""
                    <div class="gov-card" style="border-color: #FF5555; background-color: #1A0D0D; margin-bottom: 0.75rem;">
                        <div class="gov-card-title" style="color: #FF5555; font-size: 1.1rem;">
                            Confirm Deletion
                        </div>
                        <div class="gov-card-sub" style="color: #E6EDF3; margin-top: 0.5rem; font-size: 0.9rem;">
                            Are you sure you want to permanently delete the AI system <strong>{sys.name}</strong>? This action cannot be undone.
                        </div>
                    </div>
                    """,
                ...
                with btn_col1:
                    ...
                    if st.button("Delete System", key=f"confirm_top_{sys.id}", help=f"Confirm delete of '{sys.name}'"):
                        try:
                ...
                st.markdown(f"""
                    <div class="gov-card" style="border-color: #4ADE4A; background-color: #0E1117;">
                        <div class="gov-card-title" style="color:#4ADE4A; margin-bottom: 1rem;">Edit System: {sys.name}</div>
                    </div>
                """, unsafe_allow_html=True)
                ...
                with st.form(f"edit_form_{sys.id}"):
                    ...
                    with col2e:
                        ...
                        try:
                            source_index = SOURCES.index(sys.model_source) if sys.model_source in SOURCES else 0
                        except Exception:
                            source_index = 0
                        model_source_e = st.selectbox("Model Source", SOURCES, index=source_index)
                    ...
                    with col_can:
                        cancel_edit = st.form_submit_button("Cancel", on_click=lambda s_id=sys.id: st.session_state.pop(f'editing_{s_id}', None))
                ...
                if submit_edit:
                    ...
                    try:
                        ...
                        st.toast(f"System '{name_e}' updated successfully!", icon="✅")
                        st.rerun()
                    except Exception as e:
                ...
                with card_col:
                    st.markdown(
                        f"""
                        <div class="system-card-marker"></div>
